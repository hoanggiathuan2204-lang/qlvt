...
class AppDatabase {
  ...
  //--------------------------------------------------
  // INIT DATABASE — đường dẫn động, không hardcode
  //--------------------------------------------------

  Future<Database> _initDatabase() async {
    // Ưu tiên thư mục Documents nếu có, fallback về appDocuments
    Directory folder;
    try {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      // Thêm bảng mới khi nâng cấp từ version 1
      await db.execute('''
...
''');
  ...
  Future<int> updateMaterial(MaterialModel material) async {
    final db = await database;
    return db.update('materials', material.toMap(),
  ...
  Future<MaterialModel?> getMaterial(int id) async {
    final db = await database;
    final maps =
        await db.query('materials', where: 'id=?', whereArgs: [id], limit: 1);
  ...
  Future<int> addDelivery(DeliveryModel delivery) async {
    final db = await database;
    return db.insert('deliveries', delivery.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<int> updateDelivery(DeliveryModel delivery) async {
    final db = await database;
    return db.update('deliveries', delivery.toMap(),
  ...
  Future<DeliveryModel?> getDelivery(int id) async {
    final db = await database;
    final maps = await db
        .query('deliveries', where: 'id=?', whereArgs: [id], limit: 1);
    if (maps.isEmpty) return null;
  ...
  Future<int> updateSupplier(SupplierModel supplier) async {
    final db = await database;
    return db.update('suppliers', supplier.toMap(),
  ...
  Future<int> addImportHistory(ImportModel record) async {
    final db = await database;
    return db.insert('import_history', record.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<List<ImportModel>> getImportHistoryByMaterial(int materialId) async {
    final db = await database;
    final maps = await db.query('import_history',
        where: 'materialId=?', whereArgs: [materialId], orderBy: 'id DESC');
    return maps.map((e) => ImportModel.fromMap(e)).toList();
  }
  ...
  Future<int> addExportHistory(ExportModel record) async {
    final db = await database;
    return db.insert('export_history', record.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
